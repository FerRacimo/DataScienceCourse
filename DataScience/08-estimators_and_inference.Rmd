...
Annoyingly, to determine the "width" of the sampling distribution of the estimator, we need its standard error. This is generally unavailable, and is often replaced by the standard deviation **of the data set**, saled by the square root of the sample size: $SE(\hat{\theta}) \approx s/\sqrt{n}$. When this happens, we can no longer use the Normal distribution: the use of the standard deviation as a "stand-in" for the standard error causes the sampling distribution to become wider, morphing into a t-distribution with $n-1$ degrees of freedom:

$$\frac{\hat{\theta} - \theta_0}{s/\sqrt{n}} \sim t_{n-1}$$

The above is called a **t-statistic**, and it follows a t-distribution. The t-distribution is a handy distribution when performing hypothesis testing, particularly because it arises whenever we use the standard deviation of the data as a replacement for the standard error.
